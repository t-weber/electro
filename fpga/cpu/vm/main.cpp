...
{
	t_int mem_size { 4096 };
	std::optional<t_int> frame_size { std::nullopt };
	std::optional<t_int> heap_size { std::nullopt };
...
static bool run_vm(const fs::path& prog, const VMOptions& opts)
{
	...
	{
		t_int dat = vm.PopRaw<t_int>();

		std::cout << "Stack[" << stack_idx << "] = " << dat;
		std::cout << std::endl;
