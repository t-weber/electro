 ...
 */
const SymInfo* SymTab::AddSymbol(const std::string& name, t_int addr,
	t_int loc, VMType ty, bool is_func, t_int num_args)
{
	SymInfo info
	{
		.addr = addr,
...
std::streampos ConstTab::AddConst(const t_constval& constval)
{
	...
	if(std::holds_alternative<t_real>(constval))
	{
		...
		m_ostr.put(static_cast<t_byte>(VMType::REAL));
		// write real data
		m_ostr.write(reinterpret_cast<const char*>(&realval), sizeof(t_real));
	}
	else if(std::holds_alternative<t_int>(constval))
	{
		...
		m_ostr.put(static_cast<t_byte>(VMType::INT));
		// write int data
		m_ostr.write(reinterpret_cast<const char*>(&intval), sizeof(t_int));
	}
	/*else if(std::holds_alternative<t_str>(constval))
	{
		...
		m_ostr.put(static_cast<t_byte>(VMType::STR));
		// write string length
		t_int len = static_cast<t_int>(strval.length());
		m_ostr.write(reinterpret_cast<const char*>(&len), sizeof(t_int));
		// write string data
