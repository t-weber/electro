...
static int lex(struct ParserContext* ctx, t_value* lval, char* text)
{
	...
	if(longest_matching_token == TOK_INVALID)
	{
		my_printf("Invalid input in lexer: \"%s\".\n", input);
...
static int match(struct ParserContext* ctx, int expected)
{
	...
	{
		my_printf("Could not match symbol! Expected: %d, got %d.\n", expected, ctx->lookahead);
		return 0;
	}
...
static t_value add_term(struct ParserContext* ctx)
{
	...
	my_printf("Invalid lookahead in %s: %d.\n", __func__, ctx->lookahead);
	return 0.;
}
...
#endif
	if(ctx->lookahead == '+' || ctx->lookahead == '-' || ctx->lookahead == ')'
		|| ctx->lookahead == TOK_END || ctx->lookahead == ','
	...
	else if(ctx->lookahead == TOK_IDENT)
	{
		...
		if(ctx->lookahead == '(')
		{
			...
			else
			{
				...
				{
					my_printf("Invalid function call to \"%s\".\n", ident);
				}
			}
		}
		...
		else
		{
			...
			if(!sym)
			{
				my_printf("Unknown identifier \"%s\".\n", ident);
				return 0.;
			}
